...
fs_create_output_stream(size_t len){
    ...
    ret->data = (fs_byte*)fs_malloc(len);
    ret->pos = 0;
    ret->buff_len = len;
...
struct fs_output_stream*
fs_create_output_with_data( fs_byte* data, size_t len){
...
#define HITOL(v) v//((v >> 24) | (((v >> 16) & 0xff) << 8) | (((v >> 8) & 0xff) << 16) | (((v) & 0xff) << 24))
#define HSTOL(v) v//((v >> 8) | (v & 0xff << 8))


void
fs_stream_write_byte( struct fs_output_stream* stream,  fs_byte v){
    
    fs_stream_write_data(stream, (fs_byte*)&v, sizeof(fs_byte));
}

void
fs_stream_write_uint32( struct fs_output_stream* stream,  uint32_t v){
    v = HITOL(v);
    fs_stream_write_data(stream, (fs_byte*)&v, sizeof(uint32_t));
}

void
fs_stream_write_uint16( struct fs_output_stream* stream,  uint16_t v){
    v = HSTOL(v);
    fs_stream_write_data(stream, (fs_byte*)&v, sizeof(uint16_t));
}

void
fs_stream_write_int32( struct fs_output_stream* stream,  int32_t v){
    v = HITOL(v);
    fs_stream_write_data(stream, (fs_byte*)&v, sizeof(int32_t));
}

void
fs_stream_write_int16( struct fs_output_stream* stream,  int16_t v){
    v = HSTOL(v);
    fs_stream_write_data(stream, (fs_byte*)&v, sizeof(int16_t));
}

void
fs_stream_write_float( struct fs_output_stream* stream,  float v){
    fs_stream_write_data(stream, (fs_byte*)&v, sizeof(float));
}

void
fs_stream_write_double( struct fs_output_stream* stream,  double v){
    fs_stream_write_data(stream, (fs_byte*)&v, sizeof(double));
}

void
fs_stream_write_string( struct fs_output_stream* stream,  const char* data, size_t len ){
    fs_stream_write_uint16(stream, len);
    fs_stream_write_data(stream, (fs_byte*)data, len);
}
...
void
fs_stream_write_data( struct fs_output_stream* stream,  fs_byte* data, size_t len ){
    ...
    while(stream->pos + len > stream->buff_len){
        ...
        stream->data = realloc(stream->data, stream->buff_len);
    }
...
fs_stream_write_long( struct fs_output_stream* stream, long v){
    fs_stream_write_data(stream, (fs_byte*)&v, sizeof(long));
}
void
fs_stream_write_ulong( struct fs_output_stream* stream, unsigned long v){
    fs_stream_write_data(stream, (fs_byte*)&v, sizeof(unsigned long));
}
void
...
fs_output_stream_sub( struct fs_output_stream* stream, size_t start, size_t len ){
    ...
    if(len < 0xff && stream->buff_len > 0xffff){
        stream->data = realloc(stream->data, 0xff);
...
struct fs_input_stream*
fs_create_input_stream(const fs_byte* data, size_t len){
...
struct fs_input_stream*
fs_create_input_stream_with_copy(const fs_byte* data, size_t len){
...
void
fs_input_stream_set_data( struct fs_input_stream*  stream, const fs_byte* data, size_t len){
    if(stream->data){
        if(stream->copy){
            fs_free(stream->data);
        }
    }
    stream->data = (fs_byte*)data;
    stream->pos = 0;
    stream->len = len;
...
void
fs_input_stream_set_data_copy( struct fs_input_stream* stream, const fs_byte* data, size_t len){
    if(stream->data){
        if(stream->copy){
            fs_free(stream->data);
        }
    ...
    stream->data = (fs_byte*)fs_malloc(len);
    stream->pos = 0;
    stream->len = len;
    stream->copy = fs_true;
...
void
fs_input_stream_set_order( struct fs_input_stream* stream,  fs_byte order){
    stream->fs_byte_order = order;
}
...
fs_byte fs_stream_read_byte( struct fs_input_stream* stream){
    _stream_read(fs_byte, htonl);
}
